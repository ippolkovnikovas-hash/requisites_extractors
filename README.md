...
- Веб-шаблоны тянут Bootstrap с CDN, то есть без интернета выглядят голо;
  будет исправлено локальной копией.
- CLI-путь (`run_cli.py`, `batch_process.py`) пишет промежуточные файлы в
  `processed/` и `exports/` и не убирает их за собой. Сохранение станет
  опциональным (Э8). Веб-путь `/generate` этим не затронут — он ничего не
  оставляет на диске.
- Прогон `pytest` тоже пишет в `processed/` и `exports/`: тесты pipeline
  используют рабочие папки проекта вместо временных.
