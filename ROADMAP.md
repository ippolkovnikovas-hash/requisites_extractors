...
**Состояние: Э1-Э11 и Э9 закрыты.** 543 теста, покрытие 94%, `omit` содержит
один файл. CI зелёный на 3.13 и 3.14.

Осталось:

- **Э12 (документация)** — `docs/architecture.md`, `docs/extraction-notes.md`,
  `CONTRIBUTING.md`, структурное логирование. Единственный эпик, который можно
  закрыть целиком прямо сейчас.
- **Э11, замер на реальных документах** — требует входных данных от автора.
- **Пиннинг версий зависимостей** — сейчас ничего не закреплено, сборка может
  сломаться от обновления внешнего пакета.
