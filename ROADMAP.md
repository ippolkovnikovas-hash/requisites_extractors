...
- [ ] **Структурное логирование** — `document_id`, время по этапам, причина
      `needs_review`. Единственный незакрытый пункт эпика: это код, а не текст.
      `logging_config.py` — единственный файл, оставшийся в `omit`, его логично
      покрыть тестами заодно.
...
открыт и `mergeable: clean`.

### С чего продолжить

1. **Структурное логирование** (остаток Э12) — `document_id`, время по этапам,
   причина `needs_review`. Заодно закроется `logging_config.py`, единственный
   файл в `omit`. Самый очевидный следующий шаг: работа понятная и полностью
   выполнимая без внешних данных.
...
3. **Замер качества на реальных документах** (остаток Э11) — единственный пункт,
   ...
   карточек контрагентов: прогнать `scripts/batch_process.py` до и после,
   сравнить `fill_rate`. Синтетические фикстуры не подходят — они составлены под
   текущие правила. Реальные документы в репозиторий не коммитить.

Не забыть: **PR #1 не влит**, `master` по-прежнему на `2115012`.
