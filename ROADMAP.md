...
| Исправление цифр по контрольным суммам | Контрольные суммы ИНН/ОГРН и ключ ЦБ РФ №515 уже реализованы — их можно развернуть из проверки в исправление типовых OCR-подмен (О↔0, З↔3, Б↔6), с пометкой `warning` |
| Тюнинг Tesseract | Сборка строк по геометрии (`left`/`top`) с отсечением по `conf`, повторное распознавание числовых полей с `char_whitelist`, deskew через OSD, адаптивный порог вместо фиксированного 170 |
...
**Состояние на 04.09.2026: закрыты Э13 и Э14** — спринты 1 и 2 разбора
пайплайна. **561 тест, покрытие 95.13%**, `ruff` и `black` чисты.

### С чего продолжить

1. **Спринт 3 разбора: качество распознавания** (см. таблицу выше) —
   исправление цифр по контрольным суммам с пометкой `warning`; повторное
   распознавание числовых полей с `char_whitelist`; deskew через OSD и
   адаптивная бинаризация вместо фиксированного порога 170; сборка строк по
   геометрии (`left`/`top`) с отсечением по `conf`. Остаток Э12 — время по
   этапам и явная причина `needs_review` — логично делать заодно, это тоже
...
3. **Замер качества на реальных документах** (спринт 4 разбора, остаток Э11) —
   единственный пункт,
   ...
   Реальные документы в репозиторий не коммитить.
