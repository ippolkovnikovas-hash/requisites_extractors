...
**Состояние на 04.09.2026: закрыты Э13, Э14 и Э15** — спринты 1–3 разбора
пайплайна. **575 тестов, покрытие 95.17%**, `ruff` и `black` чисты.

### С чего продолжить

1. **Замер качества на реальных документах** (спринт 4 разбора, остаток Э11) —
   единственный пункт,
   который нельзя закрыть без входных данных от автора. Нужен набор реальных
   карточек контрагентов: прогнать `scripts/batch_process.py` до и после и
   сравнить. Мерить надо **field-level accuracy**, а не `fill_rate`: последний
   считает непустые поля и засчитывает склейку как заполненное поле.
   Синтетические фикстуры не подходят — они составлены под текущие правила.
   Реальные документы в репозиторий не коммитить. Тот же набор снимет и
   вопрос про исправление цифр по контрольным суммам (Э15, «рассмотрено и
   отклонено») — типичные цифровые ошибки OCR на реальных сканах покажут,
   стоит ли к этой идее возвращаться и с какими кандидатами подмен.
