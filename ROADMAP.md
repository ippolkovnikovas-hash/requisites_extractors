...
**Блокер, требует решения.** `ruff` по `app/`, `scripts/` и `tests/` проходит
чисто (23.08.2026). А вот `black --check` переформатировал бы **53 файла из
91** — код никогда не прогонялся через black, хотя он прописан в
`pyproject.toml`. Варианты: разово прогнать `black` отдельным коммитом
(большой механический диф, зато CI сразу зелёный) либо убрать `black --check`
из workflow и оставить только `ruff format`/`ruff check`. Решение за автором.
- Бейджи в README.
...
**Ближайший шаг: Э10 (CI).** Перед ним нужно решить вопрос с `black` — см.
блокер в описании эпика. Дальше по объёму работ — Э11 (качество извлечения) и
Э12 (документация), плюс остаток шага 4 в Э9: тесты на `fallback_regex`,
