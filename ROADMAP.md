...
- [ ] **Шаг 4, остаток.** `fallback_regex_service.py` (измерено 71%),
      `text_extraction_service.py` (64%), `text_normalization_service.py`
      (58%), `app/extractors/*`, `app/llm/*`, `app/ocr/*`.

      Их нельзя просто вывести из-под `omit`: с ними общий показатель падает до
      87%, то есть ниже текущего порога. Сначала тесты, потом снятие
      исключения — понижать порог ради этого нельзя.
