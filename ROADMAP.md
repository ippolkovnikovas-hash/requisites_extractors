...
**Остаётся:** удалить накопившиеся ранее 545 + 825 файлов — это разовая уборка,
...
**Состояние на 23.08.2026:** Э1 (кроме двух пунктов), Э2, Э3, Э4, Э5, Э6, Э7,
Э8 и шаги 1-4 (частично) эпика Э9 — сделаны. **395 тестов, покрытие 95%.**

**Ближайший шаг:** остаток Э1 (локальный Bootstrap вместо CDN, явная ошибка на
неизвестный `LLM_PROVIDER`) и чистка 14 замечаний `ruff` в `app/api/*`,
`app/ocr/*`, `scripts/run_cli.py`, `scripts/batch_process.py` — и то, и другое
нужно до Э10 (CI), иначе сборка сразу покраснеет.
