...
**Осталось из разбора — спринты 2-4, не начаты:**

| Что | Суть |
|-----|------|
| Недоступная Ollama роняет обработку | `LLMError` не ловится в `run_pipeline`; regex-слой в одиночку заполняет 12 полей из 16, но пользователь получает только flash «Ошибка при обработке файла» |
| Вызов Ollama без параметров | Нет `temperature: 0` (дефолт 0.8 — недетерминированная экстракция) и явного `num_ctx`; вместо `format: "json"` стоит отдавать JSON Schema из `RequisitesData.model_json_schema()` |
| `uploads/` не чистится | Веб-ветка сохраняет документ и никогда не удаляет; API-ветка сделана правильно, через `NamedTemporaryFile` + `unlink` |
| Дублирование таблиц в PDF | `page.extract_text()` уже включает содержимое таблиц, после чего `extract_tables()` добавляет их второй раз |
...
**Состояние на 04.09.2026: закрыт Э13** — спринт 1 разбора пайплайна.
**553 теста, покрытие 94.48%**, `ruff` и `black` чисты.

### С чего продолжить

1. **Спринт 2 разбора: устойчивость и приватность** (см. Э13) — `try/except
   LLMError` вокруг шага 5 с режимом «только regex» и явным баннером в форме;
   `temperature: 0` и явный `num_ctx` в вызове Ollama, JSON Schema вместо
   `format: "json"`; удаление загруженного файла в веб-ветке и разовая чистка
   `uploads/`; дублирование таблиц в `pdf_text_extractor`. Всё выполнимо без
   внешних данных. Сюда же переезжает остаток Э12 — время по этапам и явная
   причина `needs_review`.
...
3. **Замер качества на реальных документах** (остаток Э11) — единственный пункт,
