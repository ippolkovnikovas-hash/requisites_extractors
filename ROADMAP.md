...
### Э16. Спринт 4 разбора пайплайна: инструмент замера качества *(частично сделано 04.09.2026)*

Единственный пункт эпика Э11, который нельзя закрыть без входных данных от
автора, — не сам факт замера, а конкретные цифры на конкретных документах.
Построить и проверить инструмент можно и без них: реализация ниже готова и
проверена вживую на синтетической карточке, но сам замер «до/после» на
реальных документах не проводился — реальных документов пока нет.
...
- [ ] **Сам замер на реальных документах** — не проведён, инструмент ждёт
      входных данных.
...
**Состояние на 04.09.2026: закрыты Э13, Э14, Э15, частично Э16** — спринты
1–4 разбора пайплайна. **590 тестов, покрытие 95.28%**, `ruff` и `black`
чисты. PR #4–#6 влиты в `master` по цепочке.

### С чего продолжить

1. **Сам замер качества на реальных документах** (остаток Э16/Э11) —
   единственный пункт, который нельзя закрыть без входных данных от автора.
   Инструмент готов (`scripts/measure_accuracy.py` + `app/services/
   accuracy_service.py`): нужно подготовить `<имя>.expected.json` для набора
   реальных карточек контрагентов, прогнать «до» (на коммите перед Э13) и
   «после» (текущий `master`), сравнить отчёты. Реальные документы и эталоны
   в репозиторий не коммитить. Тот же набор снимет и вопрос про исправление
   цифр по контрольным суммам (Э15, «рассмотрено и отклонено») — типичные
   цифровые ошибки OCR на реальных сканах покажут, стоит ли к этой идее
   возвращаться и с какими кандидатами подмен.
2. **Пиннинг версий зависимостей** — сейчас не закреплено ничего, и сборка может
   покраснеть от обновления любого внешнего пакета. После пиннинга имеет смысл
   включить Dependabot и для pip (сейчас он только для github-actions).
3. **Повторное распознавание числовых полей с `char_whitelist`** — нужна новая
   архитектура (bbox по строке из `image_to_data`, обрезка региона, повторный
   вызов Tesseract только на нём), непроверенная без реальных фикстур с
   типичными числовыми ошибками OCR. Логично делать вместе с пунктом 1.
