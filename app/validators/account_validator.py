...
def account_control_key_ok(bik: str, account: str, account_type: str) -> bool:
    ...
    """
    return validate_account_key(bik, account, account_type) is True


# БИК подразделений Банка России (РКЦ): расчётный счёт считается по номеру РКЦ
_RKC_BIK_SUFFIXES = {"000", "001", "002"}


def validate_account_key(bik: str | None, account: str | None, account_type: str) -> bool | None:
    """
    Проверка контрольного ключа счёта по БИК.
    ...
    Для корр. счёта и для счетов в РКЦ (БИК оканчивается на 000/001/002):
    "0" + 5-6 цифры БИК + номер счёта.
    """
    ...
    if not (bik_clean.isdigit() and acc_clean.isdigit()):
        return None
    if len(bik_clean) != 9 or len(acc_clean) != _ACCOUNT_LENGTH:
        return None

    if account_type == "correspondent" or bik_clean[-3:] in _RKC_BIK_SUFFIXES:
        bank_code = "0" + bik_clean[4:6]
    elif account_type == "checking":
        bank_code = bik_clean[-3:]
    else:
        return None

    checksum = sum(
        int(digit) * weight % 10
        for digit, weight in zip(bank_code + acc_clean, _CONTROL_KEY_WEIGHTS)
    )
